         ...
         <div className="block xl:hidden">
            ...
            >
               <div className="relative flex flex-col gap-5 px-5 pb-6 mt-4 text-lg font-normal leading-6">
                  ...
                  {/*  search box */}
                  <div className="px-2 py-2 rounded-md flex items-center gap-4 border md:hidden">
                     <input
                        type="text"
                        className="outline-none text-base w-full bg-base-200 text-base-content"
                        placeholder="Search"
                     />
                     <svg
                        width="16"
                  ...
                  <div className="flex flex-col">
                     {headerData !== undefined && headerData?.map((item: any, index: number) => (
                        <SubMenu
                           item={item}
      ...
      >
         {item.child_items ? (
            <span className="text-base text-base-content/80 flex items-center justify-between gap-2">
               ...
               >
                  <svg
                     className={`fill-current w-5 h-5 transform transition-all duration-300 ease-in-out ${open ? 'rotate-180' : 'rotate-0'
                        }`}
                     xmlns="http://www.w3.org/2000/svg"
                     width="20"
                     height="20"
            ...
            <Link
               href={item.url}
               className={`link block link-hover text-base text-base-content/80 hover:text-primary transition hover:duration-300 font-work ${item.child_items
                  ? ''
                  : '!py-3 last-of-type:pb-0 first-of-type:pt-0'
                  }`}
               onClick={() => {
                  setOpen(false)
         ...
         {item.child_items && (
            ...
            >
               {item.child_items.map((subItem: any, index: number) => (
                  <li
                     key={index}
                  ...
                  >
                     {subItem.child_items ? (
                        ...
                        <Link
                           href={subItem.url}
                           className={`link block link-hover text-base text-base-content/80 hover:text-primary transition hover:duration-300 font-work ${subItem.subMenu ? '' : 'py-3'
                              }`}
                           onClick={() => {
                              setOpen(false)
                              setSidebarOpen(false)
